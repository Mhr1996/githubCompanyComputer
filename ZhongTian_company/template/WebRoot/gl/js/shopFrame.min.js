...
(function($) {
    ...
    $.fn.frame = function(options,data) {
        ...
        var defaults = {
            ...
            Sellerber_left: '.Sellerber_left',
            Sellerber_bottom: '.Sellerber_bottom',
            Sellerber_content: '.Sellerber_content',
        ...
        return this.each(function() {
            var thisBox = $(this),
            closeBtn = thisBox.find(settings.close_btn),
            showbtn = thisBox.find(settings.show_btn),
            colorbtn = thisBox.find(settings.color_btn),
            sideContent = thisBox.find(settings.content),
            position_float = thisBox.find(settings.position),
            headerheight = thisBox.find(settings.Sellerber_header),
            ...
            menunav = thisBox.find(settings.menu_nav),
            showiframe = thisBox.find(settings.show_iframe),
            Sellerberleft = thisBox.find(settings.Sellerber_left),
            bottomheight = thisBox.find(settings.Sellerber_bottom),
            content = thisBox.find(settings.Sellerber_content),
            columnlist = thisBox.find(settings.column_list);
            ...
            leftmenu.height($(window).height() - ((settings.header) + (settings.bottom)));
            style_height.height($(window).height() - 20);
            content.height($(window).height() - ((settings.header) + (settings.bottom))).css({
                "width": $(window).width() - (settings.menu),
                "margin-top": settings.header,
                "margin-left": settings.menu,
                "position": "relative"
            });
            page_list.height($(leftmenu).height()).css({
                "width": $(".h_products_list").width() - (settings.menu) - 2,
                "position": "relative",
                "float": "left"
            });
            ...
            closeBtn.bind("click",
            function() {
                leftmenu.stop(true, true).delay(0).animate({
                    width: +(0) + 'px'
                },
                "fast").css("display", "none");
                showbtn.stop(true, true).delay(0).animate({
                    width: +(settings.close_btn_width) + 'px'
                },
            ...
            function() {
                ...
                page_list.css({
                    "width": $(".h_products_list").width() - (settings.menu) - 1,
                    "position": "relative",
                    "float": "left"
                });
            ...
            if (settings.header === 0) {
                ...
                leftmenu.prepend('<div class="system_logo" style="position:relative;"><img src="' + (settings.logo_img) + '"></div></div>');
                leftmenu.prepend('<div class="user_operating"><a class="btn_dropout" href="javascript:void(0)" id="Exit_system">退出系统</a><a href="#" class="btn_usernfo" onclick="userBtn()">系统设置</a><ul class="mmul"><li id="userName">账户</li><li class="editpw" onclick="pasModify()">修改密码</li></ul></div>');
            }
            ...
            var treeView = frame.find('#menu_list');
            function toNavPos() {
                oUl.stop().animate({
                    'left': -num * 100
                },
            ...
            headerheight.find('.administrator').on('click',
            function() {
            ...
            methods();
            $(document).on('click', columnlist,
            function() {
                $(this).on('click', '.submenu li a',
                function() {
                    if ($(this).attr('name')) {
                        var bStop = false;
                        var bStopIndex = 0;
                        ...
                        if (!bStop) {
                            ...
                            iframe_box.find(".show_iframe").hide().eq(bStopIndex).show().find("iframe").attr({
                                "src": _href,
                                "data-href": _href
                            });
                        }
            ...
            $(columnlist).find('.submenu li.home').on('click',
            function() {
            ...
            function treeHtml(fileData, fileId) {
                ...
                function radelist(fileData, fileId) {
                    ...
                    children.forEach(function(item, index) {
                        _html += '<li class="home">' + '<a href="javascript:void(0)" name="' + item.url + '" title="' + item.title + '" class="iframeurl">' + '<i class="fa fa-angle-double-right"></i>' + item.title + '</a>' + '</li>';
                    });
                    _html += '</ul></dd>';
                }
                var children = getChildById(fileData, fileId);
                children.forEach(function(item, index) {
                    _html += '<dt class="nav_link" data-file-id="' + item.id + '">' + '<a href="javascript:void(0)" class="dropdown-toggle title_nav">' + '<i class="fa ' + item.icon + '"></i><span class="menu-text">' + item.title + ' </span><b class="arrow fa fa-angle-down"></b>' + '</a>' + '</dt>';
                    '' + radelist(fileData, item.id); + '';
                });
                _html += '</dl>';
                return _html;
            }
            ...
            function creatIframe(href, titleName) {
                ...
                var id_name = $(".show_iframe").each(function(i) {
            ...
            $(document).on("click", "#min_title_list li",
            function() {
            ...
            $(document).on("click", "#min_title_list li em.close_icon",
            function() {
            ...
            function() {
                ...
                $("#breadcrumbs").find(' .breadcrumb_style .breadcrumb>li.active').remove();
                $("#iframe_box").find('.show_iframe').eq(aCloseIndex).remove();
                $("#breadcrumbs").find(' .breadcrumb_style .breadcrumb>li').siblings("li:last").addClass("active");
            ...
            function() {
                ...
                $("#breadcrumbs").find(' .breadcrumb_style .breadcrumb>li').not(".active,.home").remove();
                $("#iframe_box").find('.show_iframe').not(".selected,.index_home").remove();
                num == 0 ? num = 0 : num--;
                tabNavallwidth();
            });
            $(document).on("click", "#dropdown_menu .tabCloseAll",
            function() {
                var $tabCloseAll = $("#breadcrumbs .breadcrumb li");
                if ($tabCloseAll.not(".name").length) {
                    $($tabCloseAll).not(".home").remove();
                    $("#iframe_box").find('.show_iframe').not("#Sort_link_0").remove();
            ...
            $(document).on("dblclick", "#min_title_list li",
            function() {
